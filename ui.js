...
var MENU_MARGIN = 100;
var recentPause = false;
var pauseMenuText = []
function pauseMenu() {
	if (!PAUSED && !recentPause) {
		//return
		//options
		//quit
		PAUSED = true;
		...
		var text1 = game.add.text(game.scale.width / 2, -200, "Return", {font: "48px Arial", fill:"#FFFFFF", stroke:"#000000", strokeThickness:"10", align:"center"});
		var text2 = game.add.text(game.scale.width / 2, -200, "Options", {font: "48px Arial", fill:"#FFFFFF", stroke:"#000000", strokeThickness:"10", align:"center"});
		var text3 = game.add.text(game.scale.width / 2, -200, "Quit", {font: "48px Arial", fill:"#FFFFFF", stroke:"#000000", strokeThickness:"10", align:"center"});
		text1.fixedToCamera = true
		text2.fixedToCamera = true
		text3.fixedToCamera = true
		pauseMenuText = [text1, text2, text3];
		game.add.tween(text1.cameraOffset).to({y: (game.scale.height / 2) - MENU_MARGIN}, 1000, Phaser.Easing.Exponential.Out, true);
		game.add.tween(text2.cameraOffset).to({y: (game.scale.height / 2)}, 1000, Phaser.Easing.Exponential.Out, true);
		game.add.tween(text3.cameraOffset).to({y: (game.scale.height / 2) + MENU_MARGIN}, 1000, Phaser.Easing.Exponential.Out, true);
		game.time.events.add(1000, function(){recentPause=false;}, this);
	}
}
...
function closePauseMenu() {
	if (PAUSED && !recentPause) {
		...
		//tween all texts out of screen
		for (var i = 0; i < pauseMenuText.length; i++) {
			var t = pauseMenuText[i];
			game.add.tween(t.cameraOffset).to({y: -200}, 1000, Phaser.Easing.Exponential.Out, true);
		}
		//destroy the texts (and set recentPause to false)
